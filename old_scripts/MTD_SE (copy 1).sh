...
write_methods_log() {
...

    {
        ...
        # Detected microbiome read extraction
        csv_row "Detected microbiome read extraction" "MTD_SE.sh" "extract_microbiome_reads" "$EXTRACT_MICROBIOME_READS" "Whether Kraken-classified reads are extracted for detected microbiome taxa"
        csv_row "Detected microbiome read extraction" "MTD_SE.sh" "ranking_mode" "absolute" "Read extraction uses the absolute detected microbiome ranking"
        csv_row "Detected microbiome read extraction" "MTD_SE.sh" "top_n" "$EXTRACT_MICROBIOME_READS_TOP_N" "Number of top taxa extracted; 0 means all detected taxa"
        csv_row "Detected microbiome read extraction" "MTD_SE.sh" "min_abundance" "$EXTRACT_MICROBIOME_READS_MIN_ABUNDANCE" "Minimum sample abundance required for extraction"
        csv_row "Detected microbiome read extraction" "MTD_SE.sh" "include_children" "$EXTRACT_MICROBIOME_READS_INCLUDE_CHILDREN" "Whether child taxa are included during KrakenTools read extraction"
...
# nao houve diferenca significativa entre o default (mais abaixo comentado) e o metodo a seguir 
fastp --trim_poly_x \
