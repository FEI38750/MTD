...
Then just to add the flag <code>-m [path/to/metadata.csv]</code> in the MTD bulk RNAseq command line. Such as:\
  ...
  - For users who have advanced knowledge and wants to have further complicated settings, could add or change corresponding parameters inside the source code MTD/MTD.sh, for example, by searching *# fastp* to locate the code block of fastp settings then add additional parameters according to the options on https://github.com/OpenGene/fastp#all-options or searching *# HISAT2* or *# Magic-BLAST* to the place for parameters adjust for the reads mapping.
